...
# Sensitivity analyses for ADRD survival and prediction models—Supplementary Appendix A
# - Robustness across prediction horizons
# - Early event (<=100 days) exclusion
# - Discrimination by subgroups using Model 5 predictions
...
HORIZONS <- c("3y" = 3 * 365.25, "7y" = 7 * 365.25, "9y" = 9 * 365.25)
EARLY_EVENT_DAYS <- 100
FINAL_XGB_PRED_COL <- "pred_Model5_Add_MetaboAge"
COMPUTE_AUPRC_SUBGROUPS <- TRUE
...
# --------------------------
# Helper: Build TEST for survival analysis
# --------------------------
build_survival_test <- function(df0) {
  ...
  df1 <- df0 %>%
    transmute(
      ...
      t_death_raw = suppressWarnings(as.numeric(.data[[T_DEATH_COL]])), # maybe YEARS
      t_cens_raw  = suppressWarnings(as.numeric(.data[[CENS_COL]])),    # maybe YEARS
  ...
  df1[-train_idx, , drop = FALSE]
}

test_surv <- build_survival_test(df0)
message("TEST event counts (0=censor,1=ADRD,2=death):")
print(table(test_surv$event))

# --------------------------
# Check IDs alignment with XGBoost predictions
# --------------------------
if (!("ID" %in% names(xgb_preds))) stop("XGB predictions must contain 'ID' column.")
xgb_preds <- xgb_preds %>% mutate(ID = as.character(ID))
overlap <- intersect(test_surv$id, xgb_preds$ID)
if (length(overlap) < 0.9 * nrow(test_surv))
  warning("TEST ID overlap survival/XGB predictions <90% -- check splitting.")
...
compute_shr_group <- function(df, group_var) {
  ...
  tibble(
    sHR = exp(b), CI_low = exp(b - 1.96 * se), CI_high = exp(b + 1.96 * se), p_value = 2 * pnorm(-abs(b / se))
  )
}
...
# --------------------------
# A) Horizons robustness
# --------------------------
shr_rows <- list(); thr_rows <- list(); plots <- list()
for (nm in names(HORIZONS)) {
  ...
  shr_rows[[nm]] <- tibble(
    Horizon = nm, Days = t_days,
    Group_sHR = shr_g$sHR, Group_CI_low = shr_g$CI_low, Group_CI_high = shr_g$CI_high, Group_p = shr_g$p_value,
    Cont_sHR_per_1pct = shr_c$sHR_per_1pct, Cont_CI_low = shr_c$CI_low, Cont_CI_high = shr_c$CI_high, Cont_p = shr_c$p_value
  )
  thr_rows[[nm]] <- tibble(Horizon = nm, Days = t_days, q75_threshold = q75)
  ...
  plots[[nm]] <- ggplot(cif_df, aes(x = time, y = est, color = group, fill = group)) +
    ...
    labs(
      title = paste0("Cumulative Incidence of ADRD by Risk Group (", nm, " TEST)"),
      subtitle = ann,
...
shr_by_horizon <- bind_rows(shr_rows); thr_by_horizon <- bind_rows(thr_rows)
write_csv(shr_by_horizon, file.path(OUT_DIR, "shr_by_horizon_test.csv"))
write_csv(thr_by_horizon, file.path(OUT_DIR, "riskgroup_thresholds_by_horizon.csv"))
# Panel plot, vertical arrangement
cif_panel <- plots[["3y"]] / plots[["7y"]] / plots[["9y"]]
ggsave(file.path(OUT_DIR, "cif_horizon_plots.png"),
       cif_panel, width = 9, height = 16, dpi = 500)

# --------------------------
# B) Early event exclusion robustness (<=100 days)
# --------------------------
...
if (has_early) {
  ...
  ann_sens <- paste0(
    "Exclude events ≤", EARLY_EVENT_DAYS, " days: sHR (High vs Low) = ", sprintf("%.2f", shr_sens$sHR),
  ...
  p_sens <- ggplot(cif_sens, aes(x=time, y=est, color=group, fill=group)) +
    ...
    scale_y_continuous(labels = percent_format(accuracy = 1)) +
    labs(
      title = paste0("CIF (TEST): Exclude events ≤", EARLY_EVENT_DAYS, " days"),
      subtitle = ann_sens,
      x = "Time (days)", y = "Cumulative incidence",
      color = "Risk group", fill = "Risk group"
    ) +
  ...
  ggsave(file.path(OUT_DIR, "early_event_exclusion_shr_cif.png"),
         p_sens, width = 9, height = 6, dpi = 500)

} else {
  message("No events within ", EARLY_EVENT_DAYS, " days in TEST split — skipping early exclusion sensitivity.")
}

# --------------------------
# C) Discrimination by subgroup (Model 5, TEST)
# --------------------------
pred_df <- xgb_preds %>% mutate(ID = as.character(ID))
sub_df <- df0 %>% mutate(
...
if (!FINAL_XGB_PRED_COL %in% names(pred_df)) stop("Missing prediction column: ", FINAL_XGB_PRED_COL)

auc_rows <- list(); auprc_rows <- list()

if (!all(is.na(pred_df$sex))) {
  for (s in levels(droplevels(pred_df$sex))) {
    dd <- pred_df %>% filter(sex == s)
    v <- auc_ci_delong(dd$y_test, dd[[FINAL_XGB_PRED_COL]])
    auc_rows[[paste0("Sex_", s)]] <- tibble(
      Analysis = "Sex subgroup", Group = as.character(s), AUC = v["AUC"], Lower_CI = v["L"], Upper_CI = v["U"]
    )
    if (COMPUTE_AUPRC_SUBGROUPS) auprc_rows[[paste0("Sex_", s)]] <- tibble(Analysis = "Sex subgroup", Group = as.character(s), AUPRC = auprc_value(dd$y_test, dd[[FINAL_XGB_PRED_COL]]))
  }
}
if (AGE_ONSET_COL %in% names(df0)) {
  dd_all <- pred_df %>%
    mutate(
  ...
  v <- auc_ci_delong(dd_early$y_test, dd_early[[FINAL_XGB_PRED_COL]])
  auc_rows[["EarlyOnset"]] <- tibble(Analysis = "Age of onset", Group = "Early-onset (<65)", AUC = v["AUC"], Lower_CI = v["L"], Upper_CI = v["U"])
  if (COMPUTE_AUPRC_SUBGROUPS) auprc_rows[["EarlyOnset"]] <- tibble(Analysis="Age of onset", Group="Early-onset (<65)", AUPRC=auprc_value(dd_early$y_test, dd_early[[FINAL_XGB_PRED_COL]]))
  dd_late <- dd_all %>% filter(y_test == 0 | late_case == 1)
  v <- auc_ci_delong(dd_late$y_test, dd_late[[FINAL_XGB_PRED_COL]])
  auc_rows[["LateOnset"]] <- tibble(Analysis = "Age of onset", Group = "Late-onset (≥65)", AUC = v["AUC"], Lower_CI = v["L"], Upper_CI = v["U"])
  if (COMPUTE_AUPRC_SUBGROUPS) auprc_rows[["LateOnset"]] <- tibble(Analysis="Age of onset", Group="Late-onset (≥65)", AUPRC=auprc_value(dd_late$y_test, dd_late[[FINAL_XGB_PRED_COL]]))
}
if (APOE4_COL %in% names(df0) && !all(is.na(pred_df$apoe4))) {
  for (g in levels(droplevels(pred_df$apoe4))) {
    dd <- pred_df %>% filter(apoe4 == g)
    v <- auc_ci_delong(dd$y_test, dd[[FINAL_XGB_PRED_COL]])
    auc_rows[[paste0("APOE4_", g)]] <- tibble(
      Analysis = "APOE-ε4 carrier status", Group = as.character(g), 
      AUC = v["AUC"], Lower_CI = v["L"], Upper_CI = v["U"]
    )
    if (COMPUTE_AUPRC_SUBGROUPS) auprc_rows[[paste0("APOE4_", g)]] <- tibble(Analysis="APOE-ε4 carrier status", Group=as.character(g), AUPRC=auprc_value(dd$y_test, dd[[FINAL_XGB_PRED_COL]]))
  }
}

auc_subgroups <- bind_rows(auc_rows)
write_csv(auc_subgroups, file.path(OUT_DIR, "auc_subgroups_from_holdout_preds.csv"))
if (COMPUTE_AUPRC_SUBGROUPS) {
  auprc_subgroups <- bind_rows(auprc_rows)
  write_csv(auprc_subgroups, file.path(OUT_DIR, "auprc_subgroups_from_holdout_preds.csv"))
}
...
if (nrow(auc_subgroups) > 0) {
  ...
  p_auc <- ggplot(auc_plot_df, aes(x = AUC, y = Group, xmin = Lower_CI, xmax = Upper_CI)) +
    ...
    geom_point(size = 2.8) +
    labs(
      title = "Sensitivity Analyses: AUC of Final XGBoost Model (Holdout TEST)",
      x = "AUC (DeLong 95% CI)", y = ""
    ) +
  ...
  ggsave(file.path(OUT_DIR, "auc_sensitivity_forestplot.png"),
         p_auc, width = 9, height = 10, dpi = 500)
}
